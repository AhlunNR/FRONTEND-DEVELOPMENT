...
const BADGE_ICON_MAP = {
  transaction:      <CheckCircle className="w-6 h-6" />,
  level:            <Trophy className="w-6 h-6" />,
  streak:           <Flame className="w-6 h-6" />,
  financial_health: <HeartPulse className="w-6 h-6" />,
};
...
export default function DesktopGamification() {
  ...
  if (loading) {
    return (
      <div className="w-full max-w-7xl mx-auto px-8 py-10 flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-4 border-slate-200 border-t-amber-500 rounded-full animate-spin" />
          <p className="text-muted-foreground text-sm font-medium">Memuat data gamifikasi...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-7xl mx-auto px-8 py-10 flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <button 
            ...
            className="px-4 py-2 text-sm font-bold bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-lg transition-colors"
          >
            Coba Lagi
          </button>
        </div>
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-8 py-10 space-y-8 animate-in fade-in zoom-in-95 duration-500 relative bg-background text-foreground transition-colors" id="snap-main-container">
      ...
      <AnimatedContent distance={30} delay={0.1} direction="vertical">
        <header>
          <h1 className="text-3xl font-extrabold tracking-tight">Pencapaian</h1>
              ...
              <CardContent className="p-6 flex flex-col gap-6 relative z-10">
                <div className="flex items-center gap-6">
                  <div className="w-20 h-20 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center shadow-lg border-4 border-white dark:border-slate-800 shrink-0 relative">
                    <span className="text-white font-black text-3xl">{level.current ?? 1}</span>
                  ...
                  <div className="flex-1">
                    <h3 className="text-xl font-bold">{level.name ?? 'Pemula Finansial'}</h3>
                    <p className="text-sm text-muted-foreground mt-1 flex items-center gap-1">
                      {streakDays > 0 ? `${streakDays} hari streak aktif!` : 'Mulai bangun streak-mu!'} <Sparkles className="w-4 h-4 text-amber-500" />
                    </p>
                  </div>
                </div>
                ...
                <CardTitle className="text-base flex items-center justify-between">
                  ...
                  {streakDays > 0 && (
                    <span className="flex items-center gap-1 text-xs font-bold bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 px-2 py-1 rounded-full">
                      <Flame className="w-3.5 h-3.5" /> {streakDays} Hari Beruntun!
                    </span>
                  )}
                </CardTitle>
                <CardDescription className="text-sm">
                  Misi selesai otomatis saat kamu melakukan aktivitas terkait
                </CardDescription>
              </CardHeader>
              <CardContent className="p-5 space-y-4">
                {missions.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-4">Belum ada misi hari ini.</p>
                ) : (
                  missions.map((mission) => (
                    <div key={mission.id} className="flex items-center justify-between bg-muted/50 p-3 rounded-xl border border-border">
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`text-sm font-bold ${mission.completed ? 'text-muted-foreground line-through' : ''}`}>
                          {mission.title}
                        </span>
                      </div>
                      {mission.completed ? (
                        ...
                        <span className="text-xs font-bold text-amber-500 bg-amber-100 dark:bg-amber-900/30 px-2 py-1 rounded-full">
                ...
                <div className="flex flex-col items-center text-center space-y-4">
                  ...
                  <div className="relative w-32 h-32 flex items-center justify-center">
                    <svg className="w-full h-full transform -rotate-90">
                      ...
                      <circle 
                        cx="64" cy="64" r="56" 
                        className={`${getHealthColor(healthScore)} stroke-current drop-shadow-md`}
                        strokeWidth="12" 
                        fill="none" 
                        strokeLinecap="round"
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className={`text-3xl font-black ${getHealthColor(healthScore)}`}>{healthScore}</span>
                      <span className="text-[10px] font-bold text-muted-foreground uppercase">Skor</span>
                    </div>
                  </div>
                  <div>
                    <h3 className={`text-lg font-bold ${getHealthLabelColor(healthScore)}`}>{health.label ?? 'Memuat...'}</h3>
      ...
      <AnimatedContent distance={40} delay={0.6} direction="vertical">
        <h2 className="text-lg font-bold flex items-center gap-2 mb-4 mt-8">
          <Medal className={`w-5 h-5 ${themeColor}`} /> Lencana
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {badges.map((badge) => {
            const isActive = badge.unlocked;
            const badgeIcon = BADGE_ICON_MAP[badge.type] ?? <Star className="w-6 h-6" />;
            const badgeColor = badge.color ? `bg-[${badge.color}]` : (BADGE_COLOR_MAP[badge.type] ?? 'bg-slate-500');
            ...
            return (
              <Card 
                ...
                className="border-border shadow-sm overflow-hidden cursor-pointer transition-transform hover:scale-105 active:scale-95"
              >
                <CardContent className={`p-4 flex flex-col items-center text-center gap-3 relative transition-all duration-500 ${!isActive ? 'opacity-60 grayscale' : ''}`}>
                  {isActive && (
                    <div className="absolute top-2 right-2">
                      <Sparkles className={`w-3 h-3 ${badge.type === 'level' ? 'text-amber-500' : badge.type === 'financial_health' ? 'text-emerald-500' : 'text-blue-500'} animate-pulse`} />
                    </div>
                  )}
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white shadow-inner transition-colors duration-500 ${displayColor}`}>
                    {isActive ? badgeIcon : <Lock className="w-5 h-5" />}
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-foreground leading-tight">{badge.name}</h4>
